  ...
  const handleConfirmSave = async () => {
    try {
      ...
      if (lastResponse && lastResponse.payload) {
        const result = lastResponse.payload as { message: string; result: Slot | null };
        // Use the response to set success message with more details
        setSuccessMessage(`All slots saved successfully! ${result.message}`);
      } else {
